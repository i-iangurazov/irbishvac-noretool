      ...
      <form aria-disabled={writeConnection !== "ready"} className="campaign-input-form" onSubmit={save} key={mode}>
        {mode === "plan" ? (
          <>
            ...
            <label><span>Category</span><select name="category" onChange={(event) => setSelectedPlanCategory(event.target.value)} value={selectedPlanCategory}><option value="paid">Paid channels</option><option value="separate-spend">Brand / ATL Marketing</option><option value="organic">Organic / Online Listings</option><option value="automation">Automation</option><option value="other">Other / Unmapped</option></select>{selectedPlanCategory === "separate-spend" ? <small className="marketing-brand-note">Billboards, TV, Radio</small> : null}</label>
