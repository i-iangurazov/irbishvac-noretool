...
- Launch the TV at `/kitchen?tv=1&kiosk=1&rotate=1`. Each screen advances after 20 seconds; `seconds=30` changes the interval (10–120 seconds). Previous/next and section buttons pause rotation. Play resumes it. The existing DashboardShell supplies the unchanged header, fullscreen controls, dashboard navigation, and kiosk behavior. The kitchen content uses Lucide icons. Empty news/celebration sections remain accessible manually and are skipped during unattended rotation.
